...
@Component({
  selector: 'app-pinner-add-another-bank',
...
export class AddAnotherBankComponent implements OnInit,AfterViewInit {
	...
	onSubmit(form) {
	    ...
	    if(form.valid) {
		    ...
		    console.log('fd',fd);
			//this.commonservice.postHttpCall({url:'/pinners/add-another-bank-details', data:fd, contenttype:"form-data"}).then(result=>this.submitSuccess(result));
			this.commonservice.postHttpCall({url:'/pinners/create-customer-add-bank', data:fd, contenttype:"form-data"}).then(result=>this.submitSuccess(result));
		} else {
	    	this.isformSubmitted = false;
