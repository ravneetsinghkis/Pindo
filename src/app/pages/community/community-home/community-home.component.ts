...
export class CommunityHomeComponent implements OnInit {
  ...
  mapObj: any;
  selectedtype: any;
  deletebookmark: any = '/api/pinner/delete-bookmark';
  user_location: any;
  subcategory_id: any = [];
  blog_image_url: any;
  admin_post_image_url: string = '';
  postShowFilterType = 'allType';
  pinnerDoerDetailList: any = [];
  nearest_community_array_length: any;
  ...
  ngOnInit() {
    ...
    let hireDoerSuccess = localStorage.getItem('hireDoerSuccess');
    localStorage.removeItem('hireDoerSuccess');
    if (hireDoerSuccess) {
      Swal({
        text: 'The job was awarded and the Doer has been notified!',
        type: 'success',
  ...
  addBookMark(postid, index) {
    ...
    // console.log(currentPostDetails); return;
  ...
  likePost(postid, status, index) {
    ...
    if (currentPostDetails.user) {
      ...
      let type = status == 1 ? '“liked”' : '“unliked”';
  ...
  formatPostCommentAndReplayCommentDetails(post_comments, index) {
    let comment_array = JSON.parse(JSON.stringify(post_comments));
    let comment_list = comment_array.filter(each_comment =>
  ...
  changeParentCategory() {
    ...
    this.commonservice.postCommunityHttpCall({
      ...
      data: { parent_id: this.parentcategory },
      contenttype: 'application/json'
    })
      .then(result => {
        if (result.status == 1) {
  ...
  changeChieldCategorySearch(e, id) {
    if (e.target.checked) {
      ...
      var index = this.subcategory_id.indexOf(id);
      if (index > -1) {
    ...
    this.commonservice.postCommunityHttpCall({ url: '/api/pinner/nearest-communities', data: { lat: this.user_lati, lng: this.user_lang, page: page_number, limit: this.community_activity_limit }, contenttype: 'application/json' }).then(result => {
      if (result.status == 1) {
        ...
        this.nearest_community.forEach(function (item) {
          ...
          let population = parseFloat(item.distance);
          that.maparray.push({
    ...
    this.commonservice.postCommunityHttpCall({ url: '/api/pinner/my-communities', data: { page: page_number, limit: this.community_activity_limit }, contenttype: 'application/json' }).then(result => {
      if (result.status == 1) {
        ...
        result.data.rows.forEach(resultantdata => {
          ...
          let latitude = parseFloat(resultantdata.community.latitude);
          let longitude = parseFloat(resultantdata.community.longitude);
          let population = parseFloat(resultantdata.community.distance);
          that.maparray.push({
            center: { lat: latitude, lng: longitude },
            population: population
          });
 ...
 */
  ...
  getUserLocationInformation() {
    this.commonservice.postCommunityHttpCall({ url: '/api/pinner/get-user-locations', data: {}, contenttype: 'application/json' }).then(result => {
      if (result.status == 1) {
        ...
        //   this.user_location = result.data.rows[0].locations[0].address+' '+result.data.rows[0].locations[0].city+', '+result.data.rows[0].locations[0].state+' '+result.data.rows[0].locations[0].zipcode;
   ...
   * @param community_id
   */
  connectToContact(communitynearestlist, i) {
    let link = communitynearestlist.user_type == 1 ? 'community/community-home' : 'doer/community-home';
    this.commonservice.postCommunityHttpCall({ url: '/api/pinner/connection-request', data: { contacted_user_id: communitynearestlist.id }, contenttype: 'application/json' }).then(result => {
      if (result.status == 1) {
        var postData = {
          'sender_id': this.loginUserId,
          'reciver_id': communitynearestlist.id,
          'post_id': '',
        ...
        this.myGlobals.notificationSocket.emit('post-community-notification', postData);
        // this.getNearestCommunityContactSearch();
        this.getPinnerOrDoerListApi();
        this.responseMessageSnackBar(result.msg);
      } else {
        this.responseMessageSnackBar(result.msg, 'error');
    ...
    this.commonservice.postCommunityHttpCall({
      ...
      .then(result => {
        if (result.status == 1) {
          ...
          // console.log(this.pinnerDoerDetailList);
        }
      });
  }
 ...
 */
  ...
  mapInfoOpenDialog(communityname, zipcode, latitude, longitude, communityid, type, user_community_id): void {
    ...
    let tempdialogRefconst3 = dialogRef.componentInstance.listingPopulated.subscribe(($event) => {
      this.refreshingBothNearybyAndJoinedCommunityListing();
    });
  ...
  */
  ...
  getAdminPostList() {
    this.commonservice.postCommunityHttpCall({
      ...
      if (result.status == 1) {
        ...
        let temp_admin_community_list = result.data.rows;
        let community_array_val = JSON.parse(JSON.stringify(temp_admin_community_list));
        community_array_val.forEach((array_val, community_index) => {
          ...
          comment_list.forEach((element, index) => {
            ...
            console.log("Comment log admin = ", element);
  ...
  /**
  * Gets all community list
  * @param filter_val1 (Filter Post By 1)
  * @param filter_val2  (Filter Post by 2)
  * @param parentcategory (parent category)
  * @param subcategory_id (Subcategory)
  */
  getCommunityList() {
    ...
    this.commonservice.postCommunityHttpCall({
      ...
      data: {
        ...
        filter_2_by: this.selectedall,
        filter_3_by: this.parentcategory,
        filter_4_by: this.subcategory_id,
        filter_5_by: this.filter_5_by,
        ...
        if (result.status == 1) {
          ...
          if (this.community_list) {
            ...
            community_array_val.forEach((array_val, community_index) => {
              ...
              // this.community_list[community_index].user.firstNameShow = this.commonservice.checkPrivacySettingByTypeAndValue(this.community_list[community_index].user.user_control,this.community_list[community_index].user_control_status,'first_name');

              // this.community_list[community_index].user.lastNameShow = this.commonservice.checkPrivacySettingByTypeAndValue(this.community_list[community_index].user.user_control,this.community_list[community_index].user_control_status,'last_name');

              // this.community_list[community_index].user.profilePictureShow = this.commonservice.checkPrivacySettingByTypeAndValue(this.community_list[community_index].user.user_control,this.community_list[community_index].user_control_status,'profile_photo');

              // this.community_list[community_index].user.addressShow = this.commonservice.checkPrivacySettingByTypeAndValue(this.community_list[community_index].user.user_control,this.community_list[community_index].user_control_status,'primary_address');

              // if(this.community_list[community_index].receiver){
              //   this.community_list[community_index].receiver.firstNameShow = this.commonservice.checkPrivacySettingByTypeAndValue(this.community_list[community_index].receiver.user_control,this.community_list[community_index].receiver.user_control_status,'first_name');

              //   this.community_list[community_index].receiver.lastNameShow = this.commonservice.checkPrivacySettingByTypeAndValue(this.community_list[community_index].receiver.user_control,this.community_list[community_index].receiver.user_control_status,'last_name');

              //   this.community_list[community_index].receiver.profilePictureShow = this.commonservice.checkPrivacySettingByTypeAndValue(this.community_list[community_index].receiver.user_control,this.community_list[community_index].receiver.user_control_status,'profile_photo');

              //   this.community_list[community_index].receiver.addressShow = this.commonservice.checkPrivacySettingByTypeAndValue(this.community_list[community_index].receiver.user_control,this.community_list[community_index].receiver.user_control_status,'primary_address');
              // }
              // if(this.community_list[community_index].recomended_user){
              //   this.community_list[community_index].recomended_user.firstNameShow = this.commonservice.checkPrivacySettingByTypeAndValue(this.community_list[community_index].recomended_user.user_control,this.community_list[community_index].recomended_user.user_control_status,'first_name');

              //   this.community_list[community_index].recomended_user.lastNameShow = this.commonservice.checkPrivacySettingByTypeAndValue(this.community_list[community_index].recomended_user.user_control,this.community_list[community_index].recomended_user.user_control_status,'last_name');

              //   this.community_list[community_index].recomended_user.profilePictureShow = this.commonservice.checkPrivacySettingByTypeAndValue(this.community_list[community_index].recomended_user.user_control,this.community_list[community_index].recomended_user.user_control_status,'profile_photo');

              //   this.community_list[community_index].recomended_user.addressShow = this.commonservice.checkPrivacySettingByTypeAndValue(this.community_list[community_index].recomended_user.user_control,this.community_list[community_index].recomended_user.user_control_status,'primary_address');
              // }
            ...
            // console.log("Data=    ", this.communityDetailsList);
          }
        } else {
  ...
  commentWriteOnKeyup(evnt) {
    ...
    var scroll_height = $(evnt.target).get(0).scrollHeight;
  ...
  openCommentonPost(index) {
    let community_array_val = JSON.parse(JSON.stringify(this.communityDetailsList));
    community_array_val.forEach((array_val, community_index) => {
   ...
   */
  advertisement_list() {
    this.commonservice.postCommunityHttpCall({ url: '/api/pinner/list-advertisements', data: {}, contenttype: 'application/json' }).then(result => {
      if (result.status == 1) {
  ...
  /**
    * Start a conversation dialog open
    */
   commonPostOpenDialog(): void {
    const dialogRef = this.dialog.open(CreatePostCommonDialogComponent, {
      width: '430px',
      panelClass: 'comnDialog-panel',
      autoFocus: false
      // data: {name: this.name}
  ...
  requestCommunity() {
    this.dialog.open(RequestCommunityComponent, {
      width: '600px',
      disableClose: true
    });
  }
