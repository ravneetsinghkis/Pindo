 ...
 */
  onSubmitCreatePost(val) {
    ...
    if (this.invite_pindo.invalid) {
      ...
      this.commonservice.postCommunityHttpCall({ url: '/api/pinner/pindo-join-request', data: { base_url: new_url, email: val.invite_email, name: val.invite_user_name }, contenttype: 'application/json' }).then(result => {
        if (result.status == 1) {
