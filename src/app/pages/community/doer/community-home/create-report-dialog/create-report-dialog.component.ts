 ...
 */
  ...
  */
  onSubmitCreateReport(val) {
    ...
    this.commonservice.postCommunityHttpCall({ url: '/api/pinner/add-logs',
     data: { log_type: 'post report', log_value: this.data.id, description: val.message }, contenttype: "application/json" }).then(result => {
      if (result.status == 1) {
        this.closeDialog();
        var postData = {
  ...
  wordCountFunc(event) {
    setTimeout(() => {
      const msgValLength = this.create_report.controls.message.value.trim().length;
